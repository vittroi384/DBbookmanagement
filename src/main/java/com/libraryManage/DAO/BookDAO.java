...
public class BookDAO {
	...
	public List<BookDTO> genreBook(String inputISBN,String inputGenre) {
		...
		List<BookDTO> result = jdbcTemplate.query("SELECT *  FROM (SELECT * FROM BOOK WHERE ISBN != '"+inputISBN+"' AND GENRE='"+inputGenre+"' order by dbms_random.value ) WHERE ROWNUM <= 4", (rs, rowNum) -> {
	...
	public List<BookDTO> newBook() {
		...
		List<BookDTO> result = jdbcTemplate.query("SELECT * FROM (SELECT * FROM BOOK ORDER BY BOOKDATE DESC) WHERE ROWNUM <= 10", (rs, rowNum) -> {
			BookDTO bookDTO = new BookDTO(rs.getString("ISBN"), rs.getString("TITLE"), rs.getString("AUTHOR"),
					rs.getString("GENRE"), rs.getString("PUBLISHER"), rs.getString("IMAGE"), rs.getInt("COUNT"),
					rs.getString("SUMMARY"), rs.getInt("HIT"), rs.getDate("BOOKDATE"));
			return bookDTO;
	...
	public List<BookDTO> showFive() {
		List<BookDTO> result = jdbcTemplate.query("SELECT * FROM (SELECT * FROM BOOK ORDER BY ISBN DESC) WHERE ROWNUM <= 5", (rs, rowNum) -> {
			BookDTO bookDTO = new BookDTO(rs.getString("ISBN"), rs.getString("TITLE"), rs.getString("AUTHOR"),
					rs.getString("GENRE"), rs.getString("PUBLISHER"), rs.getString("IMAGE"), rs.getInt("COUNT"),
					rs.getString("SUMMARY"), rs.getInt("HIT"), rs.getDate("BOOKDATE"));
			return bookDTO;
	...
	public void insertBook(BookDTO _bookDTO) {
		...
		jdbcTemplate.update(
				"INSERT INTO BOOK(ISBN, TITLE, AUTHOR, GENRE, PUBLISHER, IMAGE, COUNT, SUMMARY, HIT, BOOKDATE) VALUES('"
	...
	public List<BookDTO> hitBookThree() {
		List<BookDTO> result = jdbcTemplate.query("SELECT * FROM (SELECT *FROM BOOK WHERE HIT >= 5 ORDER BY HIT DESC) WHERE ROWNUM <= 3",
				(rs, rowNum) -> {
					BookDTO bookDTO = new BookDTO(rs.getString("ISBN"), rs.getString("TITLE"), rs.getString("AUTHOR"),
							rs.getString("GENRE"), rs.getString("PUBLISHER"), rs.getString("IMAGE"), rs.getInt("COUNT"),
							rs.getString("SUMMARY"), rs.getInt("HIT"), rs.getDate("BOOKDATE"));
