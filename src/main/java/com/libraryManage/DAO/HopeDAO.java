...
public class HopeDAO {
	...
	public List<HopeDTO> showAll() {
		List<HopeDTO> result = jdbcTemplate.query("SELECT * FROM REQUEST", (rs, rowNum) -> {
			HopeDTO hopeDTO = new HopeDTO(rs.getString("ISBN"), rs.getString("TITLE"), rs.getInt("REQUESTNUMBER"),
					rs.getString("LINK"));
			return hopeDTO;
		});
		return result;
	...
	public HopeDTO selectByISBN(String inputISBN) {
		try {
			return jdbcTemplate.queryForObject("SELECT * FROM REQUEST WHERE ISBN=?",
					(rs, rowNum) -> new HopeDTO(rs.getString("ISBN"), rs.getString("TITLE"), rs.getInt("REQUESTNUMBER"),
							rs.getString("LINK")),
					inputISBN);
		} catch (Exception ex) {
			return null;
	...
	public void insertHope(HopeDTO _hopeDTO) {
		...
		jdbcTemplate.update("INSERT INTO REQUEST(ISBN, TITLE, REQUESTNUMBER, LINK) VALUES('" + hopeDTO.getHopeISBN() + "', '"
